...
const BlogInfoPage = () => {
    ...
    useEffect(() => {
        if (isNew) {
            // no need to preload emptyBlog here, creation happens in handleSave
            setLoading(false);
        } else {
    ...
    const handleSave = async () => {
        try {
            ...
            if (isNew) {
                // 🔹 Create a blog with just defaults (empty fields)
                const res = await axios.post(`/api/blogs`, emptyBlog, { headers });

                // redirect immediately to edit page with new id
                router.push(`/admin/panel/blog-info/${res.data._id}`);
            } else {
